  ...
  return (
    <>
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50/80 px-3 py-1.5 text-xs text-zinc-500 hover:border-zinc-300 hover:bg-zinc-100 hover:text-zinc-900 transition-all cursor-pointer shadow-2xs"
            >
              ...
              <kbd className="hidden sm:inline-flex items-center gap-0.5 rounded border border-zinc-200 bg-white px-1.5 py-0.5 text-[10px] text-zinc-400 font-mono">
                ⌘K
              </kbd>
            </button>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-zinc-200 bg-white px-4 pt-2 pb-4 space-y-2">
            {navItems.map((item) => (
              <Link
                ...
                className={`block py-2 text-sm font-semibold ${
                  isActive(item.path) ? 'text-zinc-950 font-bold border-l-2 border-[#93d500] pl-2' : 'text-zinc-500'
                }`}
              >
                {item.label}
              </Link>
            ...
            <button
              ...
              className="flex w-full items-center gap-2 rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2 text-xs font-medium text-zinc-700"
            >
      ...
      {cmdKOpen && (
        ...
        >
          <div
            className="w-full max-w-xl rounded-xl border border-zinc-200 bg-white shadow-2xl overflow-hidden cursor-default"
          ...
          >
            <div className="flex items-center px-4 border-b border-zinc-100">
              ...
              <input
                ...
                className="w-full py-3.5 text-sm text-zinc-950 placeholder:text-zinc-400 focus:outline-none"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="text-xs font-bold text-zinc-400 hover:text-zinc-600 mr-2 cursor-pointer"
                >
                  Clear
                </button>
              )}
              <kbd className="rounded border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[10px] text-zinc-400 font-mono">ESC</kbd>
            </div>
            ...
            <div className="max-h-96 overflow-y-auto p-2 space-y-4">
                ...
                <div>
                  ...
                  {filteredAuthors.map((author) => (
                    <div
                      ...
                      className="flex items-center justify-between p-2.5 rounded-lg hover:bg-indigo-50/50 cursor-pointer transition-colors group"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-xs font-bold text-zinc-950 group-hover:text-indigo-600 transition-colors">{author.name}</div>
                          <div className="text-[11px] text-zinc-500">{author.role || author.affiliation}</div>
                        </div>
                      </div>
              ...
              {filteredPapers.length > 0 && (
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 px-3 py-1 flex items-center gap-1">
                  ...
                  {filteredPapers.map((paper) => (
                    <div
                      ...
                      className="flex items-center justify-between p-2.5 rounded-lg hover:bg-zinc-50 cursor-pointer transition-colors group"
                    >
                      <div className="pr-4">
                        <div className="text-xs font-bold text-zinc-950 group-hover:text-indigo-600 transition-colors line-clamp-1">{paper.title}</div>
                        <div className="text-[11px] text-zinc-500">{paper.venue} ({paper.year})</div>
                      </div>
                      <span className="text-[10px] rounded-full bg-zinc-100 px-2 py-0.5 font-medium text-zinc-700 shrink-0">
              ...
              {filteredPapers.length === 0 && filteredAuthors.length === 0 && (
                <div className="text-center py-8 text-xs text-zinc-500">
                  No matching publications or researchers found for "<span className="font-semibold text-zinc-900">{searchQuery}</span>"
                </div>
              )}
